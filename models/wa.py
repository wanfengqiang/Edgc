...
class WA(BaseLearner):
    ...
    def create_loss_fn(self, inputs, targets):
        ...
        def loss_fn():
            ...
            loss_clf = loss_clf_fn(logits, targets)
            old_logits = self._old_network(inputs)["logits"]
            loss_kd = loss_kd_fn(logits, old_logits, self._known_classes, self.args['T'])
...
def loss_clf_fn(logits, targets):

    return F.cross_entropy(logits, targets)


def loss_kd_fn(logits, old_logits, known_classes, temperature):
    logits = logits[:, :known_classes] / temperature
    old_logits = old_logits[:, :known_classes] / temperature

    soft_target = F.softmax(old_logits, dim=-1)
    log_probs = F.log_softmax(logits, dim=-1)

    kd_loss = F.kl_div(log_probs, soft_target, reduction='batchmean')
